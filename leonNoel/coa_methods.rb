...
def get_province_name()
	...
	print "What province code would you like to use? "
	pc = gets.chomp

	if(provinces.values.include?(pc))
		answer = provinces.index(pc) #or puts "{pc} is: #{provinces.invert[pc]}
		puts answer
	else
		puts "This is not a valid code. Vailed codes are: "
		provinces.values.each {|prov| puts prov} #or puts provinces.values
	end
end
