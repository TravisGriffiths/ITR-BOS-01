...
# JC - remember, ruby's else if syntax looks like elsif, not elseif. It's confusing, I know :/
	elseif your_guess > secret_number
		guesses_left-=1
		puts "\nPlease guess lower. Guesses left: #{guesses_left}" 

# JC - when creating an else statement, it's not necessary to put logic next to the else. In fact,
# JC - else will automatically encapsulate all conditions where your ifs and elsifs don't cover
	else your_guess < secret_number
		guesses_left-=1
		puts "\nPlease guess higher. Guesses left: #{guesses_left}"

# JC - Make sure the 'end' in the line below matches up with the same tabs as your ifs, elsifs, and elses
end
