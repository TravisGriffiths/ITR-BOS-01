...
#    a String and then return a random answer.
#
#    Start by defining a new method called `forecast_future` that
#    accepts a parameter called `question` and a parameter called
#    `responses`.

def forecast_future(question, responses)

indexes = [:positive,:neutral,:negative]
rnd_index = indexes[rand(0..2)]
rnd_size = responses[rnd_index].length()
rnd_size -= 1

answer = responses[rnd_index][rand(0..rnd_size)]
...
puts forecast_future(question, responses)
